...
doNorm = True # if True, normalize the data with MinMaxScaler
OutPutDir = "./"
anomaly_def = 'mIsMIP == False'
test_frac = 0.2
doRootToParquet = False # dump Tree to .parquet: once the .parquet file exists, set this to False to speed up data preparation
model = 'AE' # 'DBSCAN' or 'AE'
...
#____________________________________________
# Data visualization
plot_utils.plot_distr([df_mip_true, df_nucl_true], cols_to_keep, 100, ['Nucl', 'MIP'], figsize=(24, 14),
...
print (f'ML modele selected: {model}')
if model == 'DBSCAN':
    ######################################### DBSCAN #########################################
...
if model == 'AE':
    ...
    history = model.fit(df_mip_train, df_mip_train, epochs=50, batch_size=40,
    ...
    threshold = np.mean(train_loss) + 3*np.std(train_loss) # 1 std above the mean
    ...
    # plot
    df_out = pd.DataFrame({'mIsMIP': df_test_labels, 'mIsMIP_pred': preds, 'mClsizeMean': df_test[:,0]})
    print(df_out)
    ...
    hNuclPredMeanClSize.Draw("hist e same")
    hMIPPredMeanClSize.Draw("hist e same")
    canvas.SaveAs("AE_meanClSize.pdf")
